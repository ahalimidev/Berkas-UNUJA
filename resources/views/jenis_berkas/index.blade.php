...
<li class="text-gray-800">List Jenis Berkas</li>
@endsection
            ...
            <div class="card-body py-3">
                ...
                <div class="notice d-flex bg-light-primary border-primary mb-3 rounded border border-dashed p-3">
                    <div class="d-flex flex-stack">
                        <div>
                            <div class="fs-12 text-gray-700">Following is the list of Jenis Berkas data.
                            </div>
                            <div class="fs-12 text-gray-700">
                                <div style="color: #d80000; font-weight: 500;">Data Jenis Berkas is block</div>
                                <div style="color: #008c46 ; font-weight: 500;">Data Jenis Berkas is active</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        $(document).ready(function() {
            ...
            var table = $('#example').DataTable({
                ...
                rowCallback: function(row, data, dataIndex) {
                    ...
                    var rowId = data['id_jenis_berkas'];
                ...
                columns: [{
                    ...
                    },
                    {
                        data: "action",
                        render: function(data) {
                            var detail = '{{ route('jenis_berkas.show', [':jenis_berkas']) }}';
                            var edit = '{{ route('jenis_berkas.edit', [':jenis_berkas']) }}';
                            var x_edit = "";
                            var x_detail = "";
                            var x_delete = "";
                                x_detail =
                                    `<a data-toggle='tooltip' data-placement='top' title='Detail' href='${detail.replace(':jenis_berkas', data)}' class='btn btn-icon btn-bg-light btn-active-text-primary btn-sm me-1'><span class='bi bi-file-text ' aria-hidden='true'></span></a>`;
                                x_edit =
                                    `<a data-toggle='tooltip' data-placement='top' title='activate/deactivate' href='${edit.replace(':jenis_berkas', data)}' class='btn btn-icon btn-bg-light btn-active-text-primary btn-sm me-1'><span class='bi bi-pencil ' aria-hidden='true'></span> </a>`;
                                x_delete =
            ...
            $("#active").click(function() {
                if (rows_selected.length > 0) {
                    ...
                    var url = '{{ route('jenis_berkas.edit.all') }}';
                    $.ajax({
                        url: url,
                        method: "POST",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_jenis_berkas": rows_selected,
                            "status": "active",
        ...
        function deleteConfirmation(jenis_berkas) {
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: destroy.replace(':jenis_berkas', jenis_berkas),
                        method: 'DELETE',
                        data: {
