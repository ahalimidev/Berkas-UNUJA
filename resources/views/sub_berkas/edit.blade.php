...
@section('content')
    <div class="flex-column-fluid">
        <div class="card mb-5 mb-xl-8 border-2 shadow p-3 mb-5 bg-white rounded">
            ...
            <form action="{{ route('sub_berkas.update',['sub_berka' => $id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-body">
                                ...
                                <div class="flex-column mb-8 fv-row">
                                    ...
                                    <select class="selectpicker form-control form-control-sm form-select-solid"
                                        ...
                                        @foreach ($berkas as $item)
                                            @if ($item->id_berkas == $one->id_berkas)
                                                <option value="{{ $item->id_berkas }}" selected>{{ $item->nama_berkas }}</option>
                                            @else
