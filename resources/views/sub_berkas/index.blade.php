...
@section('list')
<li class="breadcrumb-item text-gray-800">Sub Sub Berkas</li>
<li class="text-gray-800">List Sub Berkas</li>
@endsection
        ...
        <div class="card mb-xl-8 mb-5 border-2">
            <div class="card-header">
                <h3 class="card-title align-items-start flex-column">
                    <span class="card-label fw-bolder fs-3 mb-1">List Sub Berkas</span>
                </h3>
                <div class="card-toolbar">
                    <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                        <a type="button" class="btn btn-sm btn-primary w-150px"
                            href="{{ route('sub_berkas.create') }}">Add Sub Berkas</a>
                    </div>
                </div>
            </div>
            <div class="card-body py-3">
                <div class="text-muted mb-3">
                    On this page is used to add, edit, and detail Sub Berkas
                </div>
                <div class="notice d-flex bg-light-primary border-primary mb-3 rounded border border-dashed p-3">
                    <div class="d-flex flex-stack">
                        <div>
                            <div class="fs-12 text-gray-700">Following is the list of Sub Berkas data.
                            </div>
                            <div class="fs-12 text-gray-700">
                                <div style="color: #d80000; font-weight: 500;">Data Sub Berkas is block</div>
                                <div style="color: #008c46 ; font-weight: 500;">Data Sub Berkas is active</div>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table id="example" class="table-bordered dt-responsive nowrap table" style="width:100%">
                        <thead>
                            <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                                ...
                                <th class="w-100px text-center">Sub Berkas</th>
                                <th class="w-100px text-center">Berkas</th>
                                <th class="w-100px text-center">Unit</th>
                                <th class="w-100px text-center">Struktur</th>
...
@section('javascript')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            var table = $('#example').DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        data: "action",
                        render: function(data) {
                            var detail = '{{ route('sub_berkas.show', [':sub_berkas']) }}';
                            var edit = '{{ route('sub_berkas.edit', [':sub_berkas']) }}';
                            var x_edit = "";
                            var x_detail = "";
                            var x_delete = "";
                                x_detail =
                                    `<a data-toggle='tooltip' data-placement='top' title='Detail' href='${detail.replace(':sub_berkas', data)}' class='btn btn-icon btn-bg-light btn-active-text-primary btn-sm me-1'><span class='bi bi-file-text ' aria-hidden='true'></span></a>`;
                                x_edit =
                                    `<a data-toggle='tooltip' data-placement='top' title='activate/deactivate' href='${edit.replace(':sub_berkas', data)}' class='btn btn-icon btn-bg-light btn-active-text-primary btn-sm me-1'><span class='bi bi-pencil ' aria-hidden='true'></span> </a>`;
                                x_delete =
            ...
            $("#active").click(function() {
                if (rows_selected.length > 0) {
                    ...
                    var url = '{{ route('sub_berkas.edit.all') }}';
                    $.ajax({
                        url: url,
                        method: "POST",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            "id_sub_berkas": rows_selected,
                            "status": "active",
                        ...
                        error: function(error) {
                            ...
                            Swal.fire('sub_berkas', 'failed to approvel', 'error')
                        }
                    })
                }
            });
        ...
        function deleteConfirmation(sub_berkas) {
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.value) {
                    var destroy = '{{ route('sub_berkas.destroy', [':sub_berkas']) }}';
                    $.ajax({
                        url: destroy.replace(':sub_berkas', sub_berkas),
                        method: 'DELETE',
                        data: {
